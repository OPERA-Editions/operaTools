...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="local:transformToRoman" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$arg = '14'">IX</xsl:when>
      <xsl:when test="$arg = '15'">XV</xsl:when>
      <xsl:when test="$arg = '16'">XVI</xsl:when>
      <xsl:when test="$arg = '17'">XVII</xsl:when>
      <xsl:when test="$arg = '18'">XVIII</xsl:when>
      <xsl:when test="$arg = '19'">IXX</xsl:when>
      <xsl:when test="$arg = '20'">XX</xsl:when>
  ...
  <xsl:template match="/">
    <xsl:element name="annot">
      ...
      <xsl:for-each select="(//tei:table[1]/tei:row)[position() > 1]"><!--  and position() &lt; 4    position()>1 | 56   position() > 47 and position() &lt; 50-->
        ...
        <xsl:variable name="cnNo" select="normalize-space(tei:cell[1])" as="xs:string"/>
        <!-- Annotation-ID -->
...
<!--        <xsl:variable name="table.numberName" select="normalize-space(tei:cell[6])" as="xs:string"/>-->
        <xsl:variable name="table.numberName" select="'0'" as="xs:string"/>
        <!-- new TE -->
        <xsl:variable name="table.section" select="normalize-space(tei:cell[6])" as="xs:string"/>
        ...
        <xsl:variable name="bar_first" select="''" as="xs:string"/>
        <xsl:variable name="bar_last" select="''" as="xs:string"/>
        ...
        <!-- Quellen -->
        <!--<xsl:variable name="sources" select="replace(normalize-space(tei:cell[12]), 'T', 'T-TE')" as="xs:string"/>-->
        <xsl:variable name="sources" select="normalize-space(tei:cell[12])" as="xs:string"/>
        
        <!-- Spottitel -->
        <xsl:variable name="spotTitle" select="normalize-space(tei:cell[10])" as="xs:string"/>
        
        <!-- Spots -->
        <xsl:variable name="spots" select="normalize-space(tei:cell[11])" as="xs:string"/>
        
        <!-- weitere IDs -->
        <!--<xsl:variable name="additionalParticipants" select="string-join(tei:cell[15]//text())" as="xs:string"/>-->
        <!--<xsl:variable name="additionalParticipants" select="'TextEdition, TE_p_003; TextEdition, TE-HenricoLeone_l_appendix-01-01; TextEdition, TE-HenricoLeone_l_appendix-01-02; TextEdition, TE-HenricoLeone_l_appendix-01-03'" as="xs:string"/>-->
        <xsl:variable name="additionalParticipants" select="string-join(tei:cell[14]//text())" as="xs:string"/>
        
        <!-- Kategorien -->
        <xsl:variable name="category" select="normalize-space(tei:cell[13])" as="xs:string"/>
...
<!--                    <xsl:variable name="spotID" select="concat('opera_zone_edition-', $editionID, '-ME_', $spotT[3])"/>-->
                    ...
                    <xsl:variable name="spotID" select="concat('opera_zone_edition-', $editionID, '-TE_', $spotT[3])"/>
                  ...
                  <xsl:variable name="additionalParticipantTSourceDocURI">
                    <xsl:choose>
                      ...
                      <xsl:otherwise>
                        ...
                        <!--<xsl:value-of select="document-uri($textSourceDocs[matches(//tei:altIdentifier/tei:idno, 'TextEdition')])"/>-->
                        <xsl:value-of select="$textSourceDocs[matches(//tei:altIdentifier/tei:idno, 'TextEdition')]"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:variable>
...
<!--                  <xsl:variable name="additionalParticipantTURI" select="concat('xmldb:exist:///db/contents/edition-74338566/texts/TextEdition.xml#', $additionalParticipantTid, ' ')"/>-->
                  <xsl:variable name="additionalParticipantTURI">
                    <xsl:choose>
                      <xsl:when test="contains($additionalParticipantTid, 'measure')">
                        <xsl:value-of select="concat('xmldb:exist:///db/contents/edition-74338566/sources/opera_source_e12fba06-b908-42db-9442-c2c8d0075fbc.xml#', $additionalParticipantTid, ' ')"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="concat('xmldb:exist:///db/contents/edition-74338566/texts/TextEdition.xml#', $additionalParticipantTid, ' ')"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:variable>
          ...
          <xsl:element name="title">
            ...
            <xsl:variable name="titleBarOrSegIndicator">
              <xsl:choose>
                ...
                <!-- only spot -->
                <xsl:when test="($bar_first = '' and $bar_last = '' and $textLine_first = '' and $textLine_last = '' and $spotTitle != '') and (contains($spotTitle, 'before') or contains($spotTitle, 'after'))">
                  <xsl:value-of select="concat ('; ', $spotTitle)"/>
                </xsl:when>
                <xsl:when test="$bar_first = '' and $bar_last = '' and $textLine_first = '' and $textLine_last = '' and $spotTitle != ''">
                  <xsl:value-of select="concat (', ', $spotTitle)"/>
                </xsl:when>
                
                <!-- Steffani-TE sections -->
                <xsl:when test="$table.section != ''">
                  <xsl:value-of select="concat('section ', $table.section, if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>
                </xsl:when>
                <xsl:when test="$table.act != '' and $textLine_first = ''">
            ...
            <xsl:variable name="actSceneLineIndicator">
              <xsl:choose>
                ...
                <xsl:when test="$table.act and $table.section">
                  ...
                  <xsl:value-of select="concat($table.act, '')"/>
                </xsl:when>
                <xsl:when test="$table.act and $textLine_first and $textLine_last">
                  <!--<xsl:value-of select="concat('(Act ', substring-before($table.actScene, '.'), ', Sc. ', substring-after($table.actScene, '.'), '; lines ', $textLine_first, '–', $textLine_last, ')')"/>-->
                  <xsl:value-of select="concat('Act ', $table.act, ',', ' Sc. ', local:transformToRoman($table.scene),'; lines ', $textLine_first, '–', $textLine_last, '')"/>
                </xsl:when>
                <xsl:when test="$table.act and $table.scene and $textLine_first">
                  <!--<xsl:value-of select="concat('(Act ', substring-before($table.actScene, '.'), ', Sc. ', substring-after($table.actScene, '.'), '; line ', $textLine_first, ')')"/>-->
                  <xsl:value-of select="concat('Act ', $table.act, ',', ' Sc. ', local:transformToRoman($table.scene),'; line ', $textLine_first, '')"/>
                </xsl:when>
                <xsl:when test="$table.act and $table.scene">
                  <xsl:value-of select="concat('Act ', $table.act, ',', ' Sc. ', local:transformToRoman($table.scene))"/>
                </xsl:when>
                <xsl:when test="$table.act">
                  <xsl:value-of select="concat($table.act, '')"/>
                </xsl:when>
            ...
            <xsl:value-of select="concat($cnNo, ' - ',
              if ($table.act and $table.section)
              then (concat($actSceneLineIndicator, ', '))
              else (),
              if ($table.act and $table.section = '' and $textLine_first = '')
              then (concat($actSceneLineIndicator, ''))
              else (),
