...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- ID of MEI work file. -->
  <xsl:variable name="workID" as="xs:string">edirom_work_a2d35700-0012-413a-be06-77feea8aff60</xsl:variable>
  
  <!-- ID of EDIROM edition file. -->
  <xsl:variable name="editionID" as="xs:string">74338565</xsl:variable>
  ...
  <xsl:variable name="textSourceDocs" select="doc(concat($pathToEditionContents, '/edition/edition_text.xml'))" as="document-node()*"/>
  ...
  <xsl:variable name="editionConcordances" as="node()*">
    <xsl:choose>
      ...
      <xsl:when test="$editionConcordanceType = 'text'">
        ...
        <xsl:copy-of select="$editionDoc//edi:work[@xml:id = $workID]//edi:concordances[@name = 'Navigation by scene/text line']"/>
      </xsl:when>
      <xsl:otherwise/>
    </xsl:choose>
  </xsl:variable>
  ...
  <xsl:template match="/">
    <xsl:element name="annot">
      ...
      <xsl:for-each select="(//tei:table[1]/tei:row)[position() > 1
        ]"><!--  and position() &lt; 4    position()>1 | 56   position() > 47 and position() &lt; 50-->
        ...
        <xsl:variable name="table.actScene" select="if ($table.act != '' and $table.scene != '') then (concat($table.act, ',', $table.scene)) else ('')" as="xs:string"/>
        ...
        <xsl:variable name="table.fullName" select="normalize-space(tei:cell[6])" as="xs:string"/>
        ...
        <xsl:variable name="bar_first" as="xs:string">
          <xsl:choose>
            ...
            <xsl:when test="$bar_first_raw = 'anacrusis'"><xsl:value-of select="'0'"/></xsl:when>
        ...
        <!-- Stimmen -->
        <!-- "ChorW (Sopr., Alto)": sopran i & ii, alt i & ii -->
        <!-- "ChorJ (Ten. I, Ten. II, Basso I, Basso II)" -->
        ...
        <xsl:variable name="parts" as="item()*">
          <xsl:for-each select="$systemT">
            <xsl:choose>
              <xsl:when test=". = 'ChorW (Sopr.,'">
                <xsl:value-of select="'Soprano (I)'"/> <!-- missing sopran ii -->
              </xsl:when>
              <xsl:when test=". = 'Chor (Sopr. I'">
                <xsl:value-of select="'Soprano (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Geister (Sopr. I'">  <!-- regular space -->
                <xsl:value-of select="'Soprano (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Geister (Sopr. I'"> <!-- protected space -->
                <xsl:value-of select="'Soprano (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Geister (Sopr. II'">
                <xsl:value-of select="'Soprano (II)'"/>
              </xsl:when>
              <xsl:when test=". = 'Sopr. II'">
                <xsl:value-of select="'Soprano (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Alto)'">
                <xsl:value-of select="'Alto (I)'"/> <!-- missing alt ii -->
              </xsl:when>
              
              <xsl:when test=". = ' ChorJ (Ten. I'">
                <xsl:value-of select="'Tenore (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Ten.'">
                <xsl:value-of select="'Tenore (I)'"/>
              </xsl:when>
              ...
              <xsl:when test=". = 'Ten. II'">
                <xsl:value-of select="'Tenore (II)'"/>
              </xsl:when>
              
              <xsl:when test=". = 'Basso'">
                <xsl:value-of select="'Basso (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Basso I'">
                <xsl:value-of select="'Basso (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Basso)'">
                <xsl:value-of select="'Basso (I)'"/>
              </xsl:when>
              <xsl:when test=". = 'Basso II)'">
                <xsl:value-of select="'Basso (II)'"/>
              </xsl:when>
              <xsl:when test=". = 'Basso II'">
                <xsl:value-of select="'Basso (II)'"/>
              </xsl:when>
              
              <xsl:when test=". = 'Tr. in Re I'">
                <xsl:value-of select="'Tromba I in '"/> <!-- not existing? -->
              </xsl:when>
              <xsl:when test=". = 'Trom. A.'">
                <xsl:value-of select="'Trombone Alto'"/>
              </xsl:when>
              <xsl:when test=". = 'Trom. T.'">
                <xsl:value-of select="'Trombone Tenore'"/>
              </xsl:when>
              <xsl:when test=". = 'Trom. B.'">
                <xsl:value-of select="'Trombone Basso'"/>
              </xsl:when>
              <xsl:when test=". = 'Fl. picc.'">
                <xsl:value-of select="''"/> <!-- not existing? -->
              </xsl:when>
              <xsl:when test=". = 'Tr. in La I'">
                <xsl:value-of select="'Tromba I in La'"/>
              </xsl:when>
              <xsl:when test=". = 'Tr. in La II'">
                <xsl:value-of select="'Tromba II in La'"/>
              </xsl:when>
              <xsl:when test=". = 'Clar. in Sib I'">
                <xsl:value-of select="'Clarinetto I in Sib'"/>
              </xsl:when>
              <xsl:when test=". = 'Clar. in Sib II'">
                <xsl:value-of select="'Clarinetto II in Sib'"/>
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mib III'">
                <xsl:value-of select="'Corno III in Mib'"/>
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mib IV'">
                <xsl:value-of select="'Corno VI in Mib'"/>
              </xsl:when>
              <xsl:when test=". = 'Cor. solo in Fa'">
                <xsl:value-of select="'Corno Solo in Fa'"/>
              </xsl:when>
              
              
              <!-- could clean up like Vl. I -->
              <xsl:when test=". = 'Fl.'">
                <xsl:value-of select="'Flauto I'"/>
              </xsl:when>
              <xsl:when test=". = 'Fl. I'">
                <xsl:value-of select="'Flauto I'"/>
              </xsl:when>
              <xsl:when test=". = 'Fl. II'">
                <xsl:value-of select="'Flauto II'"/>  <!-- not existing? -->
              </xsl:when>
              <xsl:when test=". = 'Clar. in La I'">
                <xsl:value-of select="'Clarinetto I in La'"/>
              </xsl:when>
              <xsl:when test=". = 'Clar. in La II'">
                <xsl:value-of select="'Clarinetto II in La'"/>
              </xsl:when>
              <xsl:when test=". = 'Vc.'">
                <xsl:value-of select="'Basso e Cello'"/>
              </xsl:when>
              <xsl:when test=". = 'Cb.'">
                <xsl:value-of select="'Basso e Cello'"/> <!-- probably never happening -->
              </xsl:when>
              <xsl:when test=". = 'Va.'">
                <xsl:value-of select="'Viola'"/>  <!-- not working for A3? -->
              </xsl:when>
              <xsl:when test=". = 'Fag. I'">
                <xsl:value-of select="'Fagotto I, II'"/>
              </xsl:when>
              <xsl:when test=". = 'Fag. II'">
                <xsl:value-of select="'Fagotto I, II'"/>
              </xsl:when>
              <xsl:when test=". = 'Vl. I' or . = 'Vl. I'"> <!-- space... -->
                <xsl:value-of select="'Violino I'"/>
              </xsl:when>
              <xsl:when test=". = 'Vl. II'">
                <xsl:value-of select="'Violino II'"/>
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi I'">
                <xsl:value-of select="'Corno I in Mi'"/> <!-- space... -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi I'">
                <xsl:value-of select="'Corno I in Mi'"/> <!-- space -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi II'">
                <xsl:value-of select="'Corno II in Mi'"/> <!-- space... -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi II'">
                <xsl:value-of select="'Corno II in Mi'"/> <!-- space -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi III'">
                <xsl:value-of select="'Corno III in Mi'"/> <!-- space... -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi III'">
                <xsl:value-of select="'Corno III in Mi'"/> <!-- space -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi IV'">
                <xsl:value-of select="'Corno IV in Mi'"/> <!-- space... -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Mi IV'">
                <xsl:value-of select="'Corno IV in Mi'"/> <!-- space -->
              </xsl:when>
              <xsl:when test=". = 'Cor. in Re III'">
                <xsl:value-of select="'Corno III in Re'"/>
              </xsl:when>
              <xsl:when test=". = 'Cor. in Re IV'">
                <xsl:value-of select="'Corno IV in Re'"/>
              </xsl:when>
              <xsl:when test=". = 'Fl. picc.'">
                <xsl:value-of select="'Flauto I'"/> <!-- flauto i? -->
              </xsl:when>
              <xsl:when test=". = 'Ob.'">
                <xsl:value-of select="'Oboe'"/>
              </xsl:when>
              <xsl:when test=". = 'Ob. I'"><!-- regular space -->
                <xsl:value-of select="'Oboe I'"/>
              </xsl:when>
              <xsl:when test=". = 'Ob. I'"> <!-- protected space -->
                <xsl:value-of select="'Oboe I'"/> 
              </xsl:when>
              <xsl:when test=". = 'Ob. II'">
                <xsl:value-of select="'Oboe II'"/>
              </xsl:when>
              <xsl:when test=". = 'Meph.'">
                <xsl:value-of select="'Mephistopheles'"/>
              </xsl:when>
        ...
        <!-- Quellen -->
        <xsl:variable name="sources" select="normalize-space(tei:cell[12])" as="xs:string"/>
        
        <!-- Spottitel -->
        <xsl:variable name="spotTitle" select="normalize-space(tei:cell[13])" as="xs:string"/>
        
        <!-- Spots -->
        <xsl:variable name="spots" select="normalize-space(tei:cell[14])" as="xs:string"/>
        
        <!-- weitere IDs -->
        <xsl:variable name="additionalParticipants" select="string-join(tei:cell[15]//text())" as="xs:string"/>
        
        <!-- Kategorien -->
        <xsl:variable name="category" select="if (normalize-space(tei:cell[16]) != '') then (normalize-space(tei:cell[16])) else ('M, T, S')" as="xs:string"/>
...
<!--                    <xsl:variable name="spotID" select="concat('opera_zone_edition-', $editionID, '-ME_', $spotT[3])"/>-->
                ...
                <xsl:when test="($bar_first != '') or ($textLine_first != '')">
                  ...
                  <!-- TODO: hier unterschiedliche leerzeichen beachten! -->
                  <xsl:variable name="actualMDIV" select="replace(normalize-space($table.number), ' ', ' ')"/>
...
<!-\-                        <xsl:copy-of select="concat($table.act, ',', $table.scene, ' - ', normalize-space(concat($table.number, ' ', $table.numberName)))"/>-\->
                        <xsl:copy-of select="concat($table.act, ',', $table.scene, ' - ', normalize-space($table.number))"/>
                      <xsl:otherwise>
                        <xsl:copy-of select="normalize-space($table.number)"/>
  ...
  <!--                <xsl:variable name="actualConc" select="$editionConcordances[@name = $actualMDIV]" as="element()"/>-->
                  <xsl:variable name="actualConcGroup" as="element()">
                    <xsl:choose>
                      ...
                      <xsl:when test="($bar_first = '') and ($textLine_first != '')">
                        <xsl:copy-of select="$editionDoc//edi:group[@name = 'Text line']"/>
                      </xsl:when>
                  ...
                  <xsl:variable name="concConnectionStart" as="element()">
                    <xsl:choose>
                      ...
                      <xsl:when test="($editionConcordanceType = 'music') and (($bar_first = '') and ($textLine_first != ''))">
                        <!--<xsl:copy-of select="$actualConcGroup//edi:connection[@name = $textLine_first] | $actualConcGroup//edi:connection[@name = $textLine_first]"/>-->
                  ...
                  <xsl:variable name="sourcesT" select="tokenize($sources, ', ')" as="item()*"/>
                    <xsl:choose> 
                      ...
                      <xsl:when test="$bar_first = '' and $bar_last = '' and $textLine_first != ''"> <!-- incomplete fix... -->
                        <xsl:for-each select="$sourcesT">
                            ...
                            <xsl:choose>
                              ...
                              <xsl:when test="$concPlistStartTMemberSearchSiglum = $sourceSearch">
                                <xsl:choose>
                                  ...
                                  <!-- Textsegmente sollen erstmal ignoriert werden -->
                                  <xsl:when test="contains($concPlistStartTMemberSearch, 'line-')">
                                    <xsl:text>ACHTUNG! TEXT! HIER LÄUFT ETWAS FALSCH!</xsl:text>
                                  </xsl:when>
                      ...
                      <!-- missing: $textLine_first != '' and $textLine_last != '' -->
          ...
          <xsl:element name="title">
            ...
            <xsl:variable name="titleBarOrSegIndicator">
              <xsl:choose>
                ...
                <!-- only spot -->
                <!-- No. 1 (I,3; lines 320–388), Introductory Note -->
...
<!-- check here again -->
                  <!--<xsl:value-of select="concat('Blines ', $textLine_first, '–', $textLine_last, if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>-->
                </xsl:when>
                <!-- no bars, same  segs. -->
                <xsl:when test="$bar_first = '' and $bar_last = '' and $textLine_first = $textLine_last">
                  <xsl:value-of select="concat('line ', $textLine_first, if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>
                ...
                <xsl:when test="$bar_first != '' and $bar_last = ''">
                  <xsl:value-of select="concat(if($bar_first = '0') then ('anacrusis') else (concat('bar ', $bar_first)), if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>
                </xsl:when>
                ...
                <xsl:when test="$bar_last = $bar_first">
                  <xsl:value-of select="concat('bar ', if($bar_first = '0') then ('anacrusis') else ($bar_first), if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>
                </xsl:when>
                ...
                <!-- different bars -->
                <xsl:when test="$bar_last != $bar_first" >
                  <xsl:value-of select="concat('bars ', if($bar_first = '0') then ('anacrusis') else ($bar_first), '–', $bar_last, if ($spotTitle) then (concat(', ', $spotTitle)) else ())"/>
                </xsl:when>
            ...
            <xsl:variable name="actSceneLineIndicator">
              <xsl:choose>
                ...
                <xsl:when test="$textLine_first and $textLine_last">
                  ...
                  <xsl:value-of select="concat(if($table.actScene) then (concat($table.actScene, '; ')) else (), 'lines ', $textLine_first, '–', $textLine_last)"/>
                </xsl:when>
                <!--<xsl:when test="$table.actScene and $textLine_first">-->
                <xsl:when test="$textLine_first">
                  <!--<xsl:value-of select="concat('(Act ', substring-before($table.actScene, '.'), ', Sc. ', substring-after($table.actScene, '.'), '; line ', $textLine_first, ')')"/>-->
                  <xsl:value-of select="concat(if($table.actScene) then (concat($table.actScene, '; ')) else (), 'line ', $textLine_first)"/>
                </xsl:when>
            ...
            <!-- $no, ' ', -->
            <!-- III,8; lines 1599–1600 -->
            <!-- No. 1 (I,3; lines 320–388), Introductory Note -->
            <xsl:value-of select="concat(
                                    if ($table.number)
                                    then (normalize-space($table.number))
                                    else (),
                                    
                                    if (not($table.number) and $actSceneLineIndicator)
                                    then (concat($actSceneLineIndicator, ''))
                                    else (),
                                    
                                    if ($titleBarOrSegIndicator != '')
                                    then (concat(', ', $titleBarOrSegIndicator, ''))
                                    else (),
                                    
                                    if($actSceneLineIndicator != '' and $table.number)
                                    then (concat(' (', $actSceneLineIndicator, ')'))
                                    else (),
                                    
                                    if ($spotTitle)
                                    then (concat(', ', $spotTitle))
                                    else (),
  ...
  <xsl:template match="tei:hi">
    <xsl:choose><?TODO italic+sup ?>
      ...
      <xsl:when test="contains(@rend,'superscript underline')">
        <xsl:element name="rend">
          <xsl:attribute name="rend" select="'sup underline'"/>
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="contains(@rend,'superscript doubleunderline')">
        <xsl:element name="rend">
          <xsl:attribute name="rend" select="'sup underline_double'"/>
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:when>
      ...
      <xsl:when test="contains(@rend,'doubleunderline')">
        <xsl:element name="rend">
          <xsl:attribute name="fontstyle" select="'underline_double'"/>
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="contains(@rend,'underline')">
        <xsl:element name="rend">
          <xsl:attribute name="fontstyle" select="'underline'"/>
          <xsl:apply-templates/>
        </xsl:element>
      </xsl:when>
