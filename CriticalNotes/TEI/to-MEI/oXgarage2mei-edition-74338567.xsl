...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:edi="http://www.edirom.de/ns/1.3" xmlns:uuid="java:java.util.UUID" xmlns:functx="http://www.functx.com" xmlns:local="http://edition.opera.uni-frankfurt.de/local" xmlns="http://www.music-encoding.org/ns/mei" exclude-result-prefixes="xs xd" version="3.0">
  ...
  <xsl:output indent="yes" omit-xml-declaration="yes" use-character-maps="qnames"/>
  ...
  <!-- ID of MEI work file. -->
  <xsl:variable name="workID" as="xs:string">opera_work_9ca65f23-9dc2-487b-b012-1e6e13c3dbe9</xsl:variable>
  
  <!-- ID of EDIROM edition file. -->
  <xsl:variable name="editionID" as="xs:string">74338567</xsl:variable>
  ...
  <xsl:variable name="textSourceDocs" select="collection(concat($pathToEditionContents, '/texts?select=*.xml'))" as="document-node()*"/>
  <xsl:variable name="editionDocFilePath" as="xs:string">
    <xsl:value-of select="concat($pathToEditionContents, '/', $editionIDPrefix, $editionID, '.xml')"/>
<!--    <xsl:value-of select="'/Users/tbachmann/repos/opera/edition-74338567/edition-74338567.xml'"/>-->
  </xsl:variable>
  <xsl:variable name="editionDoc" select="doc($editionDocFilePath)" as="document-node()"/>
...
<!--        <xsl:variable name="no" select="tei:cell[1]" as="xs:string"/>-->
        ...
        <!-- Annotation-ID -->
        <xsl:variable name="annotID" select="normalize-space(tei:cell[7])" as="xs:string"/>

        <!-- Akte, Szenen und Nummern -->
        <!--<xsl:variable name="table.actScene" select="normalize-space(tei:cell[3])" as="xs:string"/>-->
        <xsl:variable name="table.act" select="'NA'" as="xs:string"/>
        <xsl:variable name="table.scene" select="'NA'" as="xs:string"/>
        <!--<xsl:variable name="table.actScene" select="concat(normalize-space(tei:cell[3]), '.', normalize-space(tei:cell[4]))" as="xs:string"/>-->
        <xsl:variable name="table.actScene" select="'NA'" as="xs:string"/>
        <xsl:variable name="table.number" select="'NA'" as="xs:string"/>
        <xsl:variable name="table.numberName" select="normalize-space(tei:cell[8])" as="xs:string"/>

        <xsl:variable name="table.no.bar" select="normalize-space(tei:cell[2])" as="xs:string"/>

        <!-- Taktangaben -->
        <xsl:variable name="bar_first_raw" select="functx:substring-before-if-contains(normalize-space(tei:cell[9]), ',')" as="xs:string"/>
        <xsl:variable name="bar_first" as="xs:string">
          <xsl:choose>
            <xsl:when test="contains($bar_first_raw, 'after') or contains($bar_first_raw, 'before') or contains($bar_first_raw, 'beside')">
              <xsl:value-of select="normalize-space(' ')"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$bar_first_raw"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:variable name="bar_last_raw" select="normalize-space(tei:cell[10])" as="xs:string"/>
        <xsl:variable name="bar_last" as="xs:string">
          <xsl:choose>
            <xsl:when test="contains($bar_last_raw, 'after') or contains($bar_last_raw, 'before') or contains($bar_last_raw, 'beside')">
              <xsl:value-of select="normalize-space(' ')"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$bar_last_raw"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="textLine_first" as="xs:string">
          <xsl:choose>
            <xsl:when test="contains($textLine_first_raw, 'after') or contains($textLine_first_raw, 'before') or contains($textLine_first_raw, 'beside')">
              <xsl:value-of select="normalize-space(' ')"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$textLine_first_raw"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="textLine_last" as="xs:string">
          <xsl:choose>
            <xsl:when test="contains($textLine_last_raw, 'after') or contains($textLine_last_raw, 'before') or contains($textLine_last_raw, 'beside')">
              <xsl:value-of select="normalize-space(' ')"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$textLine_last_raw"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="system" select="'NA'" as="xs:string"/>
        ...
        <!-- Quellen -->
        <xsl:variable name="sources" select="normalize-space(tei:cell[4])" as="xs:string"/>
        ...
        <!-- Kategorien -->
        <xsl:variable name="category" select="normalize-space(tei:cell[3])" as="xs:string"/>

        <!-- Annotationstext -->
        <xsl:variable name="note" select="tei:cell[5]"/>
        ...
        <xsl:element name="annot">
          ...
          <xsl:attribute name="xml:id" select="$annotID"/>
              ...
              <xsl:choose>
                ...
                <xsl:when test="$spots != '' and $bar_first = '' and $textLine_first = ''">
                  ...
                  <xsl:for-each select="$spotsT">
                    ...
                    <xsl:variable name="spotSurfaceSourceID" select="$sourceDocs[//mei:mei//mei:surface[@xml:id = $spotSurfaceID]]/mei:mei/@xml:id"/>
                    <!-- edition-74338567/sources/opera_source_af56ff93-664f-4df2-817c-5ad6d826e850.xml -->
                    <!--                    <xsl:variable name="spotID" select="concat('opera_zone_edition-', $editionID, '-ME_', $spotT[3])"/>-->
                    <!-- opera_zone_edition-74338566-ME_0001 -->
                    <!-- opera_zone_edition-74338567_0001 -->
                    <!--<xsl:variable name="spotID" select="concat('opera_zone_edition-', $editionID, '-ME_', $spotT[3])"/>-->
...
<!--                    <xsl:variable name="spotParticipantURI" select="concat('xmldb:exist:///db/contents/', substring-after($spotSurfaceSourceDocURI, 'repos/'), '#', $spotID, ' ')"/>-->
                    <xsl:variable name="spotParticipantURI" select="concat('xmldb:exist:///db/contents/edition-74338567/sources/', $spotSurfaceSourceID, '.xml', '#', $spotID, ' ')"/>
...
<!--                        <xsl:copy-of select="concat($table.act, ' - ', normalize-space(concat($table.number, ' ', $table.numberName)))"/>-->
                        <xsl:copy-of select="$table.numberName"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:variable>
                  ...
                  <xsl:variable name="actualConcGroup" as="element()">
                    <xsl:choose>
                      ...
                      <xsl:when test="($bar_first = '') and ($textLine_first != '') and false()">
                  ...
                  <xsl:variable name="sourcesT" select="tokenize(concat($sources, ', ME'), ', ')" as="item()*"/>
                  <xsl:choose>

                    <!-- Wenn es nur einen Takt gibt -->
                    <!-- Variante 1 und 2: -->
                    <xsl:when test="($bar_first != '' and $bar_last = '')">
                      <!--  or ($bar_last = $bar_first) -->
                      <xsl:for-each select="$sourcesT">
                        ...
                        <xsl:for-each select="$concPlistStartT">
                          ...
                          <xsl:variable name="concPlistStartTMemberSearchSiglum" select="
                              doc(concat($pathToEditionContents, '/', substring-after(substring-before($concPlistStartTMemberSearch, '#'), concat($editionIDPrefix, $editionID, '/')), '/'))/mei:mei//mei:identifier[@type = 'siglum']/text() |
...
<!--                                      <xsl:variable name="nextMeasureID" select="doc(concat($basePathToEditionContents, substring-before(substring-after($concPlistStartTMemberSearch, '/contents/'), '#'), ' '))//mei:measure[@xml:id = $actualMeasureID]/following-sibling::mei:measure[1]/@xml:id"/>-->
                    ...
                    <xsl:when test="number($bar_last) > number($bar_first)">
                      ...
                      <xsl:for-each select="$sourcesT">
                        ...
                        <xsl:for-each select="$concPlistsJoinedT">
                          ...
                          <!-- Siglum des aktuellen Konkordanzteilnehmers-->
                          <xsl:variable name="concPlistsJoinedTMemberSearchSiglum" select="
                              doc(concat($pathToEditionContents, '/', substring-after(substring-before($concPlistsJoinedTMemberSearch, '#'), concat($editionIDPrefix, $editionID, '/')), '/'))/mei:mei//mei:identifier[@type = 'siglum']/text() |
              ...
              <xsl:if test="$additionalParticipants != ''">
                ...
                <xsl:for-each select="$additionalParticipantsT">
                  ...
                  <xsl:variable name="spotSurfaceSourceID" select="$sourceDocs[//mei:mei//mei:measure[@xml:id = $additionalParticipantTid]]/mei:mei/@xml:id"/>
<!--                  <xsl:variable name="additionalParticipantTURI" select="concat('xmldb:exist:///db/contents/', substring-after($additionalParticipantTSourceDocURI, 'Repos/'), '#', $additionalParticipantTid, ' ')"/>-->
                  <xsl:variable name="additionalParticipantTURI" select="concat('xmldb:exist:///db/contents/edition-74338567/sources/', $spotSurfaceSourceID, '.xml#', $additionalParticipantTid, ' ')"/>
...
<!-\-                *************************************                     -\->
          ...
          <xsl:element name="title">
            ...
            <xsl:variable name="titleBarOrSegIndicator">
              <xsl:choose>
                ...
                <xsl:when test="$bar_first = '' and $bar_last = '' and $textLine_first != '' and $textLine_last = ''">
                  <xsl:value-of select="
                      concat('line ', $textLine_first, if ($spotTitle) then
                        (concat(', ', $spotTitle))
                      else
                        ())"/>
                </xsl:when>
                ...
                <xsl:when test="$bar_first = '' and $bar_last = '' and $textLine_first != $textLine_last">
                  <xsl:value-of select="
                      concat('lines ', $textLine_first, '–', $textLine_last, if ($spotTitle) then
                        (concat(', ', $spotTitle))
                      else
                        ())"/>
                </xsl:when>
                ...
                <xsl:when test="$bar_first != '' and $bar_last = ''">
                  <xsl:value-of select="
                      concat(if ($bar_first = '0') then
                        ('anacrusis')
                      else
                        (concat('bar ', $bar_first)), if ($spotTitle) then
                        (concat(', ', $spotTitle))
                      else
                        ())"/>
                </xsl:when>
                ...
                <xsl:when test="$bar_last = $bar_first">
                  <xsl:value-of select="
                      concat('bar ', if ($bar_first = '0') then
                        ('anacrusis')
                      else
                        ($bar_first), if ($spotTitle) then
                        (concat(', ', $spotTitle))
                      else
                        ())"/>
                </xsl:when>
                ...
                <xsl:when test="$bar_last != $bar_first">
                  <xsl:value-of select="
                      concat('bars ', if ($bar_first = '0') then
                        ('anacrusis')
                      else
                        ($bar_first), '–', $bar_last, if ($spotTitle) then
                        (concat(', ', $spotTitle))
                      else
                        ())"/>
                </xsl:when>
            ...
            <!--<xsl:value-of select="
                ...
                then
                  ...
                  then
                    (concat($table.act, ',', $table.scene, ' Recitativo, '))
                  else
                    (
                ...
                then
                  ...
                  ())
                "/>-->
            <xsl:value-of select="$table.no.bar"/>
          </xsl:element>
          <xsl:choose>
  ...
  <xsl:template match="tei:figure">
    <xsl:variable name="graphic">
      <xsl:value-of select="
          if (starts-with(normalize-space(string-join(preceding-sibling::tei:p[1]//text())), '[')) then
            (substring-before(substring-after(normalize-space(string-join(preceding-sibling::tei:p[1]//text())), '['), ']'))
          else
            ()"/>
    </xsl:variable>
    <xsl:element name="p">
      <xsl:element name="fig">
        <xsl:element name="graphic">
          <xsl:attribute name="target" select="concat('edition-74338558/graphics/CR/', $graphic, '.jpg')"/>
