...
(: TODO: automate for all files? :)

(:let $annotations := doc(concat($contentsBasePath, 'resources/CN/CN_LiaV_docx.xml'))//tei:table[1]/tei:row[position() > 1 ]:)
(:let $annotations := doc(concat($contentsBasePath, 'resources/CN/ME CN final/CN_Steffani-ME_I-1.xml'))//tei:table[1]/tei:row[position() > 1 ]:)
...
return
    ...
    (:            let $spotZone := <zone xml:id="{concat('opera_zone_', $editionID, '-ME_', $spotID)}" type="operaAnnotSpotME" ulx="{$spotULX}" uly="{$spotULY}" lrx="{$spotLRX}" lry="{$spotLRY}"/>:)
    let $spotZone := <zone
        xml:id="{concat('opera_zone_', $editionID, '_', $spotID)}"
        type="operaAnnotSpot"
        ulx="{$spotULX}"
        uly="{$spotULY}"
        lrx="{$spotLRX}"
