...
for cn_doc in cn_docs:
  ...
  for i, row in enumerate(table.rows):
    ...
    if 'D' in sources:
      if additionlIDs != '':
        additionlIDs += '; '
      additionlIDs += 'D, opera_measure_68ea7be1-cec3-4dc5-bc25-ec10156a5189'

    if 'E' in sources:
      if additionlIDs != '':
        additionlIDs += '; '
      additionlIDs += 'E, opera_measure_f5701d7e-2de6-4f16-856e-60ea99b259fb'

    if 'F' in sources:
      if additionlIDs != '':
        additionlIDs += '; '
      additionlIDs += 'F, opera_measure_befbfb21-57c0-43a1-8129-58b456eca776'

    if 'G' in sources:
      if additionlIDs != '':
        additionlIDs += '; '
      additionlIDs += 'G, opera_measure_38f45303-427a-4e98-a27c-caf3c9fd18e6'

    row.cells[12].paragraphs[0].text = additionlIDs
